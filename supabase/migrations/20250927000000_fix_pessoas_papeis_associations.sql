-- Correção dos problemas de associação entre pessoas e papéis
-- Data: 2025-09-27
...
BEGIN
  ...
  SELECT id INTO papel_funcionario_id FROM papeis WHERE nome = 'funcionario' OR nome = 'Funcionario';
  SELECT id INTO papel_vendedora_id FROM papeis WHERE nome = 'vendedora' OR nome = 'Vendedora';
  SELECT id INTO papel_fornecedor_id FROM papeis WHERE nome = 'fornecedor' OR nome = 'Fornecedor';
...
-- 3. Criar função para garantir que pessoas existam em entidades_corporativas
...
BEGIN
  ...
  -- Sincronizar papéis
  INSERT INTO entidade_papeis (entidade_id, papel_id, data_inicio, ativo, created_at)
  SELECT 
    entidade_id,
    pp.papel_id,
    CURRENT_DATE,
  ...
  ON CONFLICT (entidade_id, papel_id) DO UPDATE SET
    ativo = EXCLUDED.ativo,
    data_inicio = CASE WHEN NOT entidade_papeis.ativo THEN EXCLUDED.data_inicio ELSE entidade_papeis.data_inicio END;
...
CREATE INDEX IF NOT EXISTS idx_pessoas_cpf_cnpj_search ON pessoas(cpf) WHERE cpf IS NOT NULL;
