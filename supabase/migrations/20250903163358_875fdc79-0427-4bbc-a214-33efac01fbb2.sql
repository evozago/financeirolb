...
INSERT INTO public.pessoas (
  id, nome, email, telefone, endereco, tipo_pessoa, categorias,
  cnpj, filial_id, ativo, created_at, updated_at, dados_fornecedor
)
SELECT 
...
ON CONFLICT (id) DO UPDATE SET
  ...
  cpf = COALESCE(pessoas.cpf, CASE WHEN length(regexp_replace(f.cnpj_cpf, '[^0-9]', '', 'g')) = 11 THEN f.cnpj_cpf ELSE NULL END);

-- Update CPF for pessoa_fisica from cnpj_cpf
UPDATE public.pessoas 
SET cpf = (SELECT cnpj_cpf FROM fornecedores WHERE fornecedores.id = pessoas.id AND length(regexp_replace(cnpj_cpf, '[^0-9]', '', 'g')) = 11)
WHERE tipo_pessoa = 'pessoa_fisica' AND cpf IS NULL AND '"fornecedor"'::jsonb <@ categorias;
