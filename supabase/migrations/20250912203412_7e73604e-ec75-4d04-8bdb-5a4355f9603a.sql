-- 1. Criar tabela de papéis para entidades corporativas (se não existir)
...
-- 2. Melhorar tabela parcelas_conta_pagar (se não existir)
...
-- 3. Inserir papéis padrão (se não existir)
...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM contas_pagar_corporativas) > 0 THEN
    RETURN 0; -- Já migrado
  END IF;
  ...
  FOR installment_record IN 
    ...
    GROUP BY fornecedor, descricao, COALESCE(numero_documento, ''), categoria, data_emissao, filial_id, entidade_id
  LOOP
    ...
    -- Buscar categoria
    SELECT id INTO categoria_id_var FROM categorias_produtos 
    WHERE nome = installment_record.categoria LIMIT 1;
