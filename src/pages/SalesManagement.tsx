...
export default function SalesManagement() {
  const { saveAllData, loading } = useSalesData();

  return (
    <div className="container mx-auto p-6 space-y-8">
      <div className="flex items-center justify-between">
        <SalesHeader />
        <Button onClick={saveAllData} size="lg" className="bg-green-600 hover:bg-green-700" disabled={loading}>
          <Save className="h-4 w-4 mr-2" />
          {loading ? 'Carregando...' : '💾 SALVAR TODOS OS DADOS'}
        </Button>
      </div>
