...
async function launchCurrentMonth(billId: string) {
  try {
    ...
    const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/launch-recurring-bill`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      throw new Error(result.error || 'Erro ao lançar conta recorrente');
    }
