...
export default function DashboardPayables() {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setSummary({
          ...
          totalDueThisWeek: parseFloat(String(stats.vencendo_semana || 0)),
          // Supondo que você tenha esse campo
          totalDueThisMonth: parseFloat(String(stats.vencendo_mes || 0)),
          // Ou qualquer que seja o nome correto do campo
        ...
        try {
          if (importMode === 'xml') {
            ...
            if (duplicatas.length === 0) {
              ...
              const {
                error: insertError
              } = await supabase.from('ap_installments').insert(installmentsToInsert);
              if (insertError) {
  ...
  return <div className="bg-background">
      ...
      {/* Import Modal */}
      <ImportModal open={importModalOpen} onClose={() => setImportModalOpen(false)} mode={importMode} onImport={handleImport} onDownloadTemplate={importMode === 'spreadsheet' ? handleDownloadTemplate : undefined} />
    </div>;
}